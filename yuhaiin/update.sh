...
# AD
cat anti-ad-domains.txt | sed $'s/\r$//' | sed '/^#/d' | sed 's/#.*$//g' | sed 's/# /*\./g' | sed '/^ *$/d' | sed 's/^/*./g'  | sed 's/$/ BLOCK/g' > yuhaiin_ad.conf
...
# DELETE
rm apple.china.conf google.china.conf accelerated-domains.china.conf ad_wars_hosts pglyoyo.txt ja_ad ja_ad_allow anti-ad-domains.txt
