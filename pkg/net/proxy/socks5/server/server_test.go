...
func TestResolveAddr(t *testing.T) {
	...
	x := s5c.ParseAddr(proxy.ParseAddressSplit(statistic.Type_tcp, "www.baidu.com", port))
	t.Log(s5c.ResolveAddr(bytes.NewBuffer(x)))

	x = s5c.ParseAddr(proxy.ParseAddressSplit(statistic.Type_tcp, "127.0.0.1", port))
