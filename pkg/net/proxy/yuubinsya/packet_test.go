...
func TestEncode(t *testing.T) {
	...
	data, addr, err := DecodePacket(buf.Bytes(), auth, true)
	assert.NoError(t, err)

	t.Log(data, addr)

	plainauth := NewPlainAuth([]byte{1, 2, 3, 4, 5})
	...
	data, addr, err = DecodePacket(buf.Bytes(), plainauth, true)
	assert.NoError(t, err)
	...
	data, addr, err = DecodePacket(buf.Bytes(), nil, false)
	assert.NoError(t, err)
