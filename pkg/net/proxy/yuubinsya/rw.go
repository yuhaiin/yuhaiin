...
func (r *reader) read() (int, error) {
	...
	origin := make([]byte, len(buf))
	copy(origin, buf)

	_, err = r.Open(buf[:0], r.nonce, buf, nil)
	increment(r.nonce)
	if err != nil {
		return 0, fmt.Errorf("seal length failed: %w, %d, %v, origin: %v, nonce: %v", err, len(buf), buf, origin, r.nonce)
	}
	...
	increment(r.nonce)
	if err != nil {
		return 0, fmt.Errorf("seal payload failed: %w, %d", err, len(buf))
	}
