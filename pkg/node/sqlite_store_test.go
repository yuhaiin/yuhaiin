...
func TestSqliteNodeStoreImportsAndPersists(t *testing.T) {
	...
	if err := os.WriteFile(tools.PathGenerator.Node(dir), data, 0o600); err != nil {
		t.Fatalf("write legacy node failed: %v", err)
	}

	store := NewSqliteNodeStore(tools.PathGenerator.State(dir))
	...
	sqliteStore, err := storagesqlite.Open(context.Background(), tools.PathGenerator.State(dir))
	if err != nil {
