...
func TestSqliteDBImportsLegacyConfigAndAndroidPreferences(t *testing.T) {
	...
	if err := os.WriteFile(tools.PathGenerator.Config(dir), configBytes, 0o600); err != nil {
	...
	db := NewSqliteDB(tools.PathGenerator.State(dir))
...
func TestSqliteDBBatchPersistsAcrossReopen(t *testing.T) {
	...
	dir := t.TempDir()
	path := tools.PathGenerator.State(dir)
